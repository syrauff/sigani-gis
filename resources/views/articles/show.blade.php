...
@section('content')
    ...
    <div class="max-w-4xl mx-auto px-4">
            <!-- Title -->
            <h1 class="text-4xl font-bold text-gray-900 leading-tight mb-6">{{ $article->title }}</h1>

            <!-- Metadata -->
            <div class="flex items-center text-sm text-gray-500 space-x-4 mb-8">
                <span>By <span class="font-medium text-gray-700">{{ $article->author ?? 'Admin' }}</span></span>
                <span>|</span>
                <span>Published on {{ \Carbon\Carbon::parse($article->published_at)->format('F d, Y') }}</span>
                <span>|</span>
            ...
            <!-- Content -->
            <article class="prose prose-lg max-w-none text-gray-800 leading-relaxed" style="text-align: justify;">
            ...
            <div class="mt-12 text-sm text-gray-400">
                <p>Created at: {{ \Carbon\Carbon::parse($article->created_at)->format('F d, Y H:i') }}</p>
                <p>Last updated: {{ \Carbon\Carbon::parse($article->updated_at)->format('F d, Y H:i') }}</p>
            </div>
        </div>
    </section>
    <!-- article Section -->
    <section class="py-12 pb-16 bg-white" id="article">
